...
> 設定が初期化されています。
> markdownParserをpandocに指定しなおす必要があります。  
> なお、マークダウンの動作が変更され、pandoc拡張の「:::」構文がdivブロックとして出力されない不具合が混入しました。
> この問題を回避するためには、MPE 0.8.22または0.8.27以降を使用してください。

> [!IMPORTANT]
> MPE 0.8.29で、parser.jsがQuickJS WebAssemblyサンドボックスで評価されるようになったため、目次のページ番号表示が動作しなくなっています。
> この問題を回避するためには、MPE 0.8.22またはMPE 0.8.27～0.8.28を使用してください。
...
| :----------------------- | :----------------------------------------------------------------------------------------------------- | :----------------- |
| isBypass                 | カスタマイズ処理全体をバイパスする                                                                     | true or false      |
| ignoreClass              | ナンバリングを無視する・柱を表示しない見出しのクラス名                                                         | 'noNumbering'      |
| headFrom                 | ナンバリングする開始見出しレベル                                                                       | 'h2'               |
| headDepth                | ナンバリングする見出しレベルの深さ                                                                     | 3                  |
| classPrefixList          | sectionに付けるクラス名                                                                                | ['chapter']        |
|                          |                                                                                                        |                    |
